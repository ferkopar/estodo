 ...
 BEGIN
  ...
  LOOP
    ...
    IF v_id_col IS NOT NULL THEN
      ...
      v_command := v_command || '      :new.CRD := SYSTIMESTAMP;      --LÉTREHOZÁS IDŐPONTJA '|| CHR (10);
      ...
      v_command := v_command || '      THEN'|| CHR (10);
      v_command := v_command || '      :new.MDD := SYSTIMESTAMP;      --MÓDOSÍTÁS IDŐPONTJA'|| CHR (10);
      ...
      v_command := NULL;
    ELSE
      DBMS_OUTPUT.PUT_LINE ('Nincs elsődleges kulcs:'||t.table_name );
    END IF;
  END LOOP;
END; 
