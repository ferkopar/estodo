...
p_typ DM_SUPER_TYP.super_typ_id % TYPE DEFAULT NULL) --egyelőre subjectiv-nél
...
WHEN NO_DATA_FOUND THEN INSERT INTO TREATM_PARAM (
  TREATM_PARAM_ID, 
  -- EPI_ID, 
  PARAM_TYPE_ID, VALUE, UNIT_TYPE_ID, DESCRIPTION,
  --   STAT_TYPE_ID,
  --   TREATM_PARAM1_ID, 
  SUBJ_TYPE_ID, ORDER_NO, SUBJ_ID, VALUE_TYPE_ID,
  --MM_ID,
  TREATM_ID
)
VALUES (SEQ_BASE.NEXTVAL, 
  -- v('EPI_ID'), 
  rec.PARAM_TYPE_ID, rec.VALUE, rec.UNIT_TYPE_ID, rec.DESCRIPTION,
  --  rec.STAT_TYPE_ID,
  -- rec.TREATM_PARAM_ID, 
  rec.SUBJ_TYPE_ID, rec.ORDER_NO, rec.SUBJ_ID, rec.VALUE_TYPE_ID,
  --MM_ID,
  P_TREATM_ID2); END;
END IF;
...
function ones(p_number NUMBER) return varchar2 as
...
begin
   if p_number = 1 then 
      ...
      return 'három';
   elsif p_number = 4 then 
      return 'négy';
   elsif p_number = 5 then 
      return 'öt';
   elsif p_number = 6 then 
      return 'hat';
   elsif p_number = 7 then 
      return 'hét';
   elsif p_number = 8 then 
      return 'nyolc';
   elsif p_number = 9 then 
      return 'kilenc';
   else
      return '';
   end if;
...
function hundreds(p_number NUMBER) return varchar2 as
...
function tens(p_number NUMBER , p_has_ones BOOLEAN default true) return varchar2 as
...
FUNCTION NUMBER_TO_TEXT_3 (p_number NUMBER) RETURN VARCHAR2 AS

  v_int_part number := floor(p_number);
  v_frac_part number  := p_number-floor(p_number);
  v_int_txt number := to_char(v_int_part);
  
  v_text varchar2(4000) := '';
  
  BEGIN
   v_text := v_text || hundreds(substr(v_int_txt,-3,1));
   v_text := v_text || tens(substr(v_int_txt,-2,1),substr(v_int_txt,-1) <> '0');
   v_text := v_text || ones(substr(v_int_txt,-1));
   return v_text;
  END NUMBER_TO_TEXT_3;



FUNCTION NUMBER_TO_TEXT (p_number NUMBER) RETURN VARCHAR2 AS

  v_int_part number := floor(p_number);
  v_frac_part number  := p_number-floor(p_number);
  v_int_txt varchar2(4000) := to_char(v_int_part);
 
  
  v_text varchar2(4000) := '';
  v_tousands varchar2(4000) := '';
  v_mills varchar2(4000) := '';
  v_miliards varchar2(4000) := '';
  v_tousand_milliards varchar2(4000) := '';

  
  BEGIN
      v_tousands :=substr(v_int_txt,1,LENGTH(v_int_txt)-3);
      v_mills := substr(v_int_txt,1,LENGTH(v_int_txt)-6);
      v_miliards := substr(v_int_txt,1,LENGTH(v_int_txt)-9);
      v_tousand_milliards := substr(v_int_txt,1,LENGTH(v_int_txt)-12);
    if LENGTH(v_miliards) <> 0 then
      v_text := v_text ||NUMBER_TO_TEXT_3(v_miliards);
      v_text := v_text || 'milliárd';
      v_text := v_text || '-';
   end if;
     if LENGTH(v_mills) <> 0 then
      v_text := v_text ||NUMBER_TO_TEXT_3(v_mills);
      v_text := v_text || 'millió';
      v_text := v_text || '-';
   end if;
   if LENGTH(v_tousands) <> 0 then
      v_text := v_text ||NUMBER_TO_TEXT_3(v_tousands);
      v_text := v_text || 'ezer';
      if v_tousands > '1' then
          v_text := v_text || '-';
      end if;
   end if;
   v_text := v_text ||NUMBER_TO_TEXT_3(substr(v_int_txt,-3));
   return v_text;
  END NUMBER_TO_TEXT;
