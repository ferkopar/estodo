...
CURSOR TREATMS IS 
 ...
 lookup_super_type_name("TREATM_TYPE_ID") TREATM_CATEGORY_NAMe,
 FROM_DATE,
 TO_DATE 
from TREATM
where TREATM_TYPE_ID = p_treatm_type_id ;

BEGIN
  HTP.P('<TABLE id="TreatmTable" class="responstable" >');
 HTP.P('<TR>');
 HTP.P('<TH colspan = 13>');
 HTP.P(lookup_super_type_name(p_treatm_type_id));
-- HTP.P(' '||p_treatm_type_id);
...
CURSOR TREATMS IS
 ...
 TO_DATE 
from TREATM
where (p_treatm_group_id is not null and TREATM_GROUP_ID = p_treatm_group_id)
   or (p_treatm_category_id is not null and treatm_type_id = p_treatm_category_id)
   or (p_treatm_group_id is null and p_treatm_category_id is null);

BEGIN
  HTP.P('<TABLE id="TreatmTable" class="responstable" >');
 HTP.P('<TR>');
 HTP.P('<TH colspan = 13>');
 HTP.P(p_treatm_category_id);
 IF p_treatm_category_id is null then
    HTP.P(lookup_super_type_name(p_treatm_group_id));
 ELSE
    HTP.P(lookup_super_type_name(p_treatm_category_id));
 END IF;
 HTP.P('<span onclick="goToEdit(0)" id = "spanAdd" class="hot"  style="float:right" >');
 HTP.P('<img src="'||v('WORKSPACE_IMAGES')|| 'Add Green Button.png">');
 HTP.P('</span>');
...
-- GENERATE_HTML.GET_SUBJECT_POPUP();
 ...
 IF l_treatm_row.treatm_group_id != 1604 then
  GENERATE_HTML.SEND_SELECT_LIST(l_treatm_row.treatm_type_id,l_treatm_row.treatm_group_id,'TREATM','TREATM_TYPE_ID',l_is_fact);
...
HTP.P('<TR>');
 ...
 HTP.P('</TR>');
  ...
  if l_treatm_row.treatm_group_id = 1604 then
     HTP.P('<TR>');
     HTP.P('<TH>');
--    HTP.P(lookup_super_type_name(l_treatm_row.TYPE1_ID));
    HTP.P('</TH>');
...
--     GENERATE_HTML.SEND_SUBJ_LIST(l_treatm_row.TYPE1_ID,l_treatm_row.SUBJ1_ID );
     ...
     HTP.P('</TD>');
     HTP.P('<TH>');
--    HTP.P(lookup_super_type_name(l_treatm_row.TYPE2_ID));
    HTP.P('</TH>');
