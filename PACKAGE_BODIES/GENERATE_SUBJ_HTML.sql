...
--    EXIT;
--  END LOOP;
  htp.p(lookup_super_type_name(p_subject.SUBJ_TYPE_ID));
  htp.p('</td>');
  htp.p('</tr>');
END PRINT_SUBJ_ROW;
...
type v_subject_type
IS
  TABLE OF SUBJECT%rowtype INDEX BY pls_integer;
...
AS
  ...
  IS
    SELECT SUBJECT.SUBJ_ID                             AS LINK,
      ...
      SUPER_TYP_TAPI.lookup_super_type_name(SUBJECT.SUBJ_TYPE_ID) KATEGORIA
...
PROCEDURE GENERATE_SUBJ_COMBO(p_subj_type number)
as
  cursor subjects is
    select * from SUBJECT WHERE SUBJ_TYPE_ID  = p_subj_type;
begin
    HTP.P('<SELECT>');
    for subject in subjects loop
      HTP.P('<OPTION value = '|| subject.subj_id ||'>'|| subject.subj_name ||'</OPTION>');
    end loop;
    HTP.P('</SELECT>');
end;


PROCEDURE GENERATE_SUBJ_TYPE_COMBO
  as
  cursor subj_types is
    select * from DM_SUPER_TYP 
     WHERE GROUP_ID = 150;
  begin
    HTP.P('<SELECT>');
    for subj_type in subj_types loop
      HTP.P('<OPTION value = '|| subj_type.super_typ_id ||'>'|| subj_type.super_type_name ||'</OPTION>');
    end loop;
    HTP.P('</SELECT>');
end;   


PROCEDURE GENERATE_SUBJ_REL_ROW(p_subj_rel_row_id number default null) as
v_subj_rel_row subj_rel%rowtype;

begin

HTP.P('<TR>');
HTP.P('<TD>');
  HTP.P('<img title="Törlés" id ="imgChildDel"   onclick="DelChild(this);" src="'||v('WORKSPACE_IMAGES')|| 'Minus Red Button.png">');
HTP.P('</TD>');
IF p_subj_rel_row_id is not null then
    select * into v_subj_rel_row FROM SUBJ_REL where SUBJ_REL_ID = p_subj_rel_row_id;
    HTP.P('<TD>');HTP.P(v_subj_rel_row.SUBJ_ID1);HTP.P('</TD>');
    HTP.P('<TD>');HTP.P(SUBJECT_TAPI.GET_SUBJECT_NAME(v_subj_rel_row.SUBJ_ID1));HTP.P('</TD>');
--    HTP.P('<TD>');HTP.P(v_subj_rel_row.REL_TYPE_ID);HTP.P('</TD>');
--    HTP.P('<TD>');HTP.P(  DM_SUPER_TYP_TAPI.LOOKUP_NAME(v_subj_rel_row.REL_TYPE_ID));HTP.P('</TD>');   
ELSE
    HTP.P('<TD>');HTP.P(seq_base.nextval);HTP.P('</TD>');
    HTP.P('<TD>');
    GENERATE_SUBJ_COMBO(101);
    HTP.P('</TD>');
--    HTP.P('<TD> </TD>');
--    HTP.P('<TD>');
--    GENERATE_SUBJ_TYPE_COMBO;
--    HTP.P('</TD>');
END IF;
HTP.P('</TR>');
END;
...
PROCEDURE GENERATE_SUBJ_REL_EDITOR(p_parent_id number, p_rel_type_id number default null)
  as
  cursor childs is
   select *
--    REL_TYPE_ID,
--       DM_SUPER_TYP_TAPI.LOOKUP_NAME(REL_TYPE_ID) REL_TYPE_NAME,
--       SUBJ_ID1,
--       SUBJECT_TAPI.GET_SUBJECT_NAME(SUBJ_ID1) SUBJECT_NAME
  from SUBJ_REL
  where FROM_DATE <= SYSDATE
    AND TO_DATE >= SYSDATE
    AND SUBJ_ID2 = p_parent_id
    AND (p_rel_type_id is NULL OR REL_TYPE_ID = p_rel_type_id);
    
BEGIN
  HTP.P('<TABLE id="SubjRelTable" class="responstable" >');
  HTP.P('<TR>');
  HTP.P('<TH  colspan = 13>');
  HTP.p(subject_tapi.get_subject_name(p_parent_id));
  HTP.P('kapcsolódó alanyok');
  HTP.p(SUPER_TYP_TAPI.LOOKUP_SUPER_TYPE_NAME(p_rel_type_id));
  HTP.P('<span id = "spanAddChild"  style="float:right;width:auto" >');
--  HTP.P('<img title="'||SUPER_TYP_TAPI.LOOKUP_SUPER_TYPE_NAME('BACK')||'" id ="imgBack" onclick="goBack();" src="'||v('WORKSPACE_IMAGES')|| 'left_32.png">');
  HTP.P('<img title="'||SUPER_TYP_TAPI.LOOKUP_SUPER_TYPE_NAME('SAVE')||'" id ="imgChildAdd"   onclick="AddChild();" src="'||v('WORKSPACE_IMAGES')|| 'Add Green Button.png">');
  HTP.P('<img title="'||SUPER_TYP_TAPI.LOOKUP_SUPER_TYPE_NAME('HELP')||'" id ="imgFootHelp"  style="float:right"   onclick="callHelpPopup();" src="'||v('WORKSPACE_IMAGES')|| 'help.png">');
  ...
  for child in childs loop
    GENERATE_SUBJ_REL_ROW(child.subj_rel_id);
  end loop;
  HTP.P('</TABLE>');

END;

PROCEDURE GENERATE_SUBJ_REL_GROUP_LIST(p_subj_id number) 
AS
    cursor childs is
   select DISTINCT REL_TYPE_ID,DM_SUPER_TYP_TAPI.LOOKUP_NAME(REL_TYPE_ID) REL_TYPE_NAME
--    REL_TYPE_ID,
--       DM_SUPER_TYP_TAPI.LOOKUP_NAME(REL_TYPE_ID) REL_TYPE_NAME,
--       SUBJ_ID1,
--       SUBJECT_TAPI.GET_SUBJECT_NAME(SUBJ_ID1) SUBJECT_NAME
...
--  HTP.P('<img title="'||SUPER_TYP_TAPI.LOOKUP_SUPER_TYPE_NAME('BACK')||'" id ="imgBack" onclick="goBack();" src="'||v('WORKSPACE_IMAGES')|| 'left_32.png">');
  ...
  for child in childs loop
    HTP.P('<TD>');
    HTP.P('<img title="'||SUPER_TYP_TAPI.LOOKUP_SUPER_TYPE_NAME('HELP')||'" id ="imgEditRel" onclick="goEditRel('||p_subj_id||','||child.REL_TYPE_ID||');" src="'||v('WORKSPACE_IMAGES')|| 'File_Edit.png">');
    HTP.P('</TD>');
    HTP.P('<TD>');HTP.P(child.REL_TYPE_ID);HTP.P('</TD>');
    HTP.P('<TD>');HTP.P(  DM_SUPER_TYP_TAPI.LOOKUP_NAME(child.REL_TYPE_ID));HTP.P('</TD>');   
  end loop;
  HTP.P('</TABLE>');
...
BEGIN
  ...
  END IF;
  --GENERATE_SUBJ_REL_EDITOR(p_subj_id);
  GENERATE_SUBJ_REL_GROUP_LIST(p_subj_id);
...
BEGIN
  ...
  ELSE
    SELECT * INTO l_subject_row FROM SUBJECT WHERE SUBJ_ID = p_subj_id;
...
AS
  ...
  IS
    SELECT * FROM SUBJECT WHERE SUBJ_TYPE_ID = p_group_id order by SUBJ_NAME ;
...
--
  ...
  if p_subj_id is not null and p_subj_id <> 0 then
     begin
      -- HACK valójában az exception ban van a jó jód, akkorról maradt, amikor még a subject-ben nem volt subj_type_id hanem cagtegory.
      select distinct value into v_subj_type_id from subj_PARAM 
        where PARAM_TYPE_ID = 1010 
        and subj_id = p_subj_id;
      exception WHEN NO_DATA_FOUND THEN
        select  SUBJ_TYPE_ID into v_subj_type_id from subject 
        where subj_id = p_subj_id;
      end;    
  else
  ...
  IF V('APP_USER') = 'FPARDI' THEN
    htp.p( 'subj_id ='||p_subj_id|| ' subj_type_id ='||v_subj_type_id );
  END IF;
