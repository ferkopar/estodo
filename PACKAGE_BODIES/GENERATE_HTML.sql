...
-- HTP.P('<TH ><a title="This is some information for our tooltip." class="tooltip">PARAM_VISIBLE</a></TH>');
 ...
 HTP.P('<TH width="40px"><div data-tooltip="Kötelező" class="tooltip">K</div></TH>');
...
PROCEDURE GET_TREATM_ROW_FORM (p_treatm_id TREATM.TREATM_ID%TYPE,
                               ...
                               p_treatm_type_id TREATM.TREATM_TYPE_ID%TYPE default null) AS  
...
-- IF l_treatm_row.treatm_group_id != 1604 then
--  SEND_SELECT_LIST(null,l_treatm_row.treatm_group_id,'TREATM','TREATM_CATEGORY_ID',l_is_fact);
...
-- HTP.P('</TD>');
 ...
 HTP.P('<TD id="TREATM_TYPE_ID" colspan = "3">');
 IF l_treatm_row.treatm_group_id != 1604 then
   SEND_SELECT_LIST(l_treatm_row.treatm_type_id,3800,l_is_fact);
...
--     HTP.P('<TH>');HTP.P(lookup_super_type_name(l_treatm_row.TYPE2_ID));HTP.P('</TH>');
     HTP.P('<TH>');HTP.P('Alany');HTP.P('</TH>');

     HTP.P('<TD id = "SUBJ2_ID">');
     --(l_treatm_row.TYPE2_ID,l_treatm_row.SUBJ2_ID );
